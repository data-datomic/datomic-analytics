...
;;; # make-instance:around (_rdfsClass), see RdfsCore file
;;;   - ensure slot definitions for this <class>, then domain constraints are also taken care
...
;;; # shared-initialize :around (_rdfsClass)
;;; # shared-initialize:after(galaxy)
...
(defmethod make-instance :around ((class (eql rdfs:Class)) &rest initargs)
  (cond ((notany #'(lambda (cls) (cl:subtypep cls (load-time-value (symbol-value 'rdfs:Resource))))
                 (getf initargs :direct-superclasses))
         (setf (getf initargs :direct-superclasses)
           (append (getf initargs :direct-superclasses) (list (load-time-value (symbol-value 'rdfs:Resource)))))
         (apply #'call-next-method class initargs))
        (t (call-next-method)))
  )

(defmethod make-instance :around ((class (eql rdfs:Datatype)) &rest initargs)
  (cond ((notany #'(lambda (cls) (cl:subtypep cls (load-time-value (symbol-value 'rdfs:Literal))))
                 (getf initargs :direct-superclasses))
         (setf (getf initargs :direct-superclasses)
           (append (getf initargs :direct-superclasses) (list (load-time-value (symbol-value 'rdfs:Literal)))))
         (apply #'call-next-method class initargs))
        (t (call-next-method)))
  )
  ...
  (let ((deletes (remove-if #'(lambda (domain)
    ...
    (loop for domain in deletes
        ...
        do (let ((direct-slotds (mop:class-direct-slots domain)))
             ...
             (setf (slot-value domain 'excl::direct-slots)
  ...
  (cond ((and (null slot-names) (not (null initargs)))
         (setq initargs
               (loop for (initarg val) on initargs by #'cddr
                   when (or (not (property? initarg))                ; if not property
                            (not (slot-exists-p instance initarg))   ; or no slot
                            (not (slot-boundp instance initarg))     ; or no slot value
                            (not (typecase val                       ; or different slot value, then ok
                                   (uri-namedspace (equal (slot-value instance initarg) val))
                                   (gx:uri (equal (slot-value instance initarg) val))
                                   (net.uri:uri (net.uri:uri= (slot-value instance initarg) val))
                                   (otherwise (equal (slot-value instance initarg) val)))))
                   append (list initarg val)))
      ...
      (cond ((eq slot-names t)  ; new definition
             (loop for (role filler) on initargs by #'cddr
                 ...
                 do (type-option-check-with-cardinality
                     instance
                     filler
                     (find role (mop:class-slots class) :key #'name)
                     nil)))
            ((null slot-names)  ; reinitialize
             (loop for (role filler) on initargs by #'cddr
                 when (property? role)
                 do (let (oldval)
                      (when (and (eq role 'owl:oneOf)
                                 (slot-exists-p instance role)
                                 (slot-boundp instance role)
                                 ;; not set equal
                                 (setq oldval (slot-value instance role))
                                 (not (and (subsetp oldval filler :test #'owl-same-p)
                                           (subsetp filler oldval :test #'owl-same-p))))
                        (warn "~S value ~S of ~S is doubly specified to ~S" role oldval instance filler)))
                   (type-option-check-with-cardinality
                    instance
                    filler
                    (find role (mop:class-slots class) :key #'name)
                    (and (slot-exists-p instance role)  ; when ordinal property
                         (slot-boundp instance role) (slot-value instance role)))))
            ))))
    ...
    (typecase type
      ...
      (cons (case (op type)
              (and (mapc #'(lambda (type)
                             (satisfy-filler instance name filler type  nil nil oldval slotd))
                     (cdr type)))
              (or (error "Not Yet!"))
...
(defun update-instance-for-unshadowing (shadow? class supers)
  "updates mclasses of all instances of class, if mclasses includes shadow-classes that is unshadowable."
  (when (shadow-class-p shadow?)
  ...
  (let ((classes (cond ((eq (car new-class) 'and) (cdr new-class))
    ...
    (cond ((length=1 classes)
          ...
          (t (setq classes (most-specific-concepts-by-superclasses classes))
             (let ((shadow (make-shadow (class-of instance) classes)))
...
(defmethod change-class ((instance rdfs:Resource) (new-class rdfs:Class) &rest initargs)
  (let ((old-class (class-of instance)))
    ...
    (cond ((and (shadow-class-p old-class) (not (shadow-class-p new-class)))
          ...
          ((and (cl:typep old-class 'shadow-class) (cl:typep new-class 'shadow-class))
          ...
          ; )
          ((cl:typep instance 'shadow-class)
           (warn "Attempting to refine ~S to ~S" instance new-class) ; by smh
           (call-next-method)  ; by smh
           )
                 ...
                 (when (and (cl:typep eslotd 'Property-effective-slot-definition)
                   ...
                   (cond ((eq old-class |rdfs:Resource|)
                          ...
                          (let* ((prop (symbol-value name))
                                 (slotd (find old-class (property-slotds prop)
                                              :key #'slot-definition-subject-type)))
                            (setf (property-slotds prop)
                              (remove slotd (property-slotds prop)))))))))))))
...
;; direct instances and mclasses handling
(defmethod update-instance-for-different-class :after ((previous galaxy) current &rest initargs)
  ;(format t "~%UPDATE-INSTANCE-FOR-DIFFERENT-CLASS:AFTER(galaxy) ~S ~S ~S" previous current initargs)
  ;; This method is called when change-class is invoked.
  (cond ((cl:typep previous rdfs:Class))
        ((or (not (rdf-class-p previous))
             (slot-boundp previous 'excl::direct-superclasses))
         (let ((old-class (class-of previous))
               (new-class (class-of current)))
           ;; class direct instances handling
           (cond ((cl:typep (class-of old-class) rdfs:Class)
                  (setf (class-direct-instances old-class)
                    (remove current (class-direct-instances old-class) :test #'eq))))
           ;; mclasses handling
           (when (and (not (eql new-class old-class)) (cl:subtypep new-class old-class))
             (cond ((shadow-class-p new-class)
                    (unless (member old-class (mop:class-direct-superclasses new-class))
                      (when (member old-class (mclasses current))
                        (error "Check!"))
                      ;(format t "~%REMOVING ~S from ~S" old-class (mclasses current))
                      (setf (mclasses current) (remove old-class (mclasses current)))
                      ))
                   (t (when (member old-class (mclasses current))
                        (error "Check!"))
                      ;(format t "~%REMOVING ~S from ~S" old-class (mclasses current))
                      (setf (mclasses current) (remove old-class (mclasses current)))
...
;;
#|
(defmethod update-instance-for-different-class :after (previous (current rdf:Property) &rest initargs)
  (let ((range (getf initargs 'rdfs:range)))
    (when range
      (format t "~%SLOTDS:~S RANGE:~S" (slot-value current 'slotds) (slot-value current 'rdfs:range)))))
|#
;;
;;
;;
...
(defmethod collect-all-instances-of ((class rdfs:Class)) ;smh
  (remove-duplicates
   (append (mklist (class-direct-instances class))
           (loop for sub in (mop:class-direct-subclasses class)
               append (cond ((eq sub (find-class '_rdfsClass)) nil)
                            (t (collect-all-instances-of sub)))))))
#|
(defmethod collect-all-instances-of ((class metaRDFSclass)) ; when class is _rdfsClass
  nil)
|#
