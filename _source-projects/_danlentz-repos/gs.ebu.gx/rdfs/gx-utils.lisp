...
;; 2009.10.16    collect-direct-instances is renamed to collect-direct-instances-of.
;; 2009.09.04    name RDFSclass is changed to _rdfsClass.
;; 2008.12.11    resource-p is renamed to rdf-object-p.
;; 2003.11.29    File created, this file is separated from Rdfs.cl
;;
...
(defun list-all-entity-uris ()
  ...
  (let (uris)
    (do-all-entity-uris
        #'(lambda (x) (when (and (uri-boundp x) (uri-value x)) (push x uris))))
    uris))
...
(defmethod list-all-entities-in ((namespace string) &optional uri?)
  ...
  (list-all-entities-in (uri namespace) uri?))
...
(defun get-form (resource)
  ...
  `(,(or (resource-type-tag resource) (type-of resource))
...
(defun tree->list (mop fn visited)
  ...
  (cond ((eq mop (find-class '_rdfsClass)) nil)
...
(defun path-filler (mop path)
  ...
  (cond ((null mop) nil)
        ((and (symbolp mop) (object? mop) (cl:typep (symbol-value mop) 'gx::shadow-class))
         (car (mop:class-direct-superclasses (symbol-value mop))))
        ((and (rsc-object-p mop) (cl:typep mop 'gx::shadow-class))
...
(defun collect-all-subproperties (property)
  (cons property
        (mappend #'collect-all-subproperties (mklist (subproperty-of property)))))
  ...
  (remove-duplicates 
   (append (collect-direct-subtypes class)
           (loop for sub in (mop:class-direct-subclasses class)
               append (cond ((eq sub (find-class '_rdfsClass)) (list rdfs:Class))
...
#|
(defmethod collect-direct-instances-of ((class metaRDFSclass))  ;seiji
  ;; class = _rdfsClass, metaclass of rdfs:Class
  ;; _rdfsClass has no such slots in SWCLOS
  nil)  
|#
...
(defmethod all-instances-generator ((class rdfs:Class)) ; added smh
  (let ((pending-classes (list class))
    ...
    (flet ((generator ()
                      (loop
                        (when pending-instances (return-from generator (pop pending-instances)))
...
(defun all-role-names (&optional (ns *default-namespace*))
  ...
  (let ((pkg
         (typecase ns
           ...
           (string (uri-namedspace-package (get-uri-namedspace (uri ns))))
  ...
  (let ((pkg
    ...
    (labels ((%all-individuals (cls)
                               (when (and (not (eql cls (load-time-value (find-class 'rdfs:Class))))
                                          (not (eql cls (load-time-value (find-class '_rdfsClass)))))
...
(defun concept-parents (concept)
  (etypecase concept
    ...
    (rdfs:Datatype )
    (rdfs:Class )
    (net.uri:uri (concept-parents (uri-value concept)))
    (consp )))
...
(defun %get-hasfiller-inherited (class role)
  (loop for slotd in (mop:class-slots class) with found
      when (and (cl:typep slotd 'OwlProperty-effective-slot-definition)
                (setq found
                      (find-if #'(lambda (ty)
                                   (and (cl:typep ty 'fills)
                                        (eq (cg:name slotd) role)))
                               (mklist (mop:slot-definition-type slotd)))))
...
(defmethod put-value ((object rdfs:Resource) (role rdf:Property) value)
  (let ((pname (name role)))
    (if (slot-exists-p object pname) (setf (slot-value object pname) value)
      (progn
