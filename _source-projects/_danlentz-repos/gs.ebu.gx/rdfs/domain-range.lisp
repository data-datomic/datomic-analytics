...
;; History
;; -------
;; 2009.10.16    rdf-object-p is renamed to rsc-object-p.
;; 2009.09.04    name RDFSclass is changed to _rdfsClass.
;; 2009.01.07    domain and range part is separated from RdfsKernel module.
...
;; (eval-when (:execute :load-toplevel :compile-toplevel collect-ranges)
...
(defpackage :gx
  (:export domain-value get-domain collect-domains domainp rangep range-value
           get-range collect-ranges  *autoepistemic-local-closed-world*))
...
;;; Note that the direct class of rdfs:Class is (virtually) rdfs:Class (actually _rdfsClass in lisp), 
;;; but the superclass of rdfs:Resource is (virtually) null (actually gx::galaxy in lisp).
...
(defun domains-satisfied-p (instance domains)
  ...
  (every #'(lambda (d)
             (or ;(not (slot-boundp instance 'mclasses)) ; in newly defining
                 (cl:typep instance d)
...
(defun %slot-value-range-check (role value range)
  ...
  (etypecase value
    ...
    (net.uri:uri (cond ((typep value range) value)               ; OK
                       ...
                       ((typep (uri-value value) range) value)  ; check 
