...
#+darwin
(defun process-status (process &key wait)
  (multiple-value-bind (pid status)
    (sb-posix:waitpid (process-pid process) (if wait 0 1))
    (when (plusp pid) status)))

#+darwin
(export 'process-status)

;; (sb-posix::waitpid (sb-impl::process-pid (sb-ext:run-program "/bin/sleep" '("5") :wait nil)) 0)
