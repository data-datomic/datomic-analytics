...
(defun get-file-kind (file follow-p)
  (let ((namestring (file-path-namestring file)))
    (handler-case
        (let ((mode (osicat-posix:stat-mode
                     (if follow-p
                         (osicat-posix:stat namestring)
                       (osicat-posix:lstat namestring)))))
          (cond
            ((nix:s-isdir mode) :directory)
            ((nix:s-ischr mode) :character-device)
            ((nix:s-isblk mode) :block-device)
            ((nix:s-isreg mode) :regular-file)
            ((nix:s-islnk mode) :symbolic-link)
            ((nix:s-issock mode) :socket)
            ((nix:s-isfifo mode) :fifo)
            (t (bug "Unknown file mode: ~A." mode))))
