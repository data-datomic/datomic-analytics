...
(defn client [region system]
  (prn "client: " )
  (prn region)
  (prn system)
  (let [args (datomic-arg-map region system)
        dat-cli (d/client args)]
    (prn (str "dat-cli: " dat-cli))
    dat-cli
  )
                       )

 (defn db-conn-from-args [region system db-name]
   (prn (str "region: " region))
   (prn (str "system: " system))
   (prn (str "db-name: " db-name))
   (let [c (client region system)

         con (d/connect c {:db-name db-name})]
    (prn (str "c " c))
     con)
   )
