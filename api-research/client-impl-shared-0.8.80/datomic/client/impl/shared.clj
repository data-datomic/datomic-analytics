...
(defprotocol AsyncOps
  (-async-op [_ conn op-spec requester m])
  (-chunked-async-op [_ conn op-spec requester m]))
...
(def op-requirements
  {:datomic.catalog/administer-system {:db-name string?
   ...
   :index-pull {:index keyword? :selector identity :start coll?}
...
(deftype Db
  ...
  (index-pull [this m] (-chunked-async-op client conn :index-pull this m))
...
(deftype Connection
  ...
  (qseq [this m] (-chunked-async-op client this [:q :qseq] this m))
  (sync [this t]
    (assert (int? t))
    (-advance-t this {:t t})
...
(defn- op-spec->op
  "op-spec should be a tuple of [wire-op dispatch-op] or a scalar wire-op
  wire-op is the op that is sent over the wire to the server
  dispatch-op is internally used to map response data.
  Returns wire-op"
  [op-spec]
  (if (coll? op-spec)
    (first op-spec)
...
(deftype Client
  ...
  AsyncOps
  (-async-op
   [_ conn op-spec requester m]
   (let [op (op-spec->op op-spec)
         request-context (-get-request-context requester)
     ...
     (-> (api->client-req op request-context m)
         ...
         (convert #(client-resp->api op-spec requester %)))))
  ...
  (-chunked-async-op
   ...
   (let [op (op-spec->op op-spec)
     ...
     (-> (api->client-req op request-context m)
         ...
         (add-routing spi)
         (send-with-retry request-context http-client spi (:timeout m) cio/http-resp->client-resp)
         (convert #(client-resp->api op-spec requester %) http-client spi request-context (:timeout m) retc))
     retc))
...
(defmethod api->client-req* :datoms
  ...
  (let [{:keys [index components]} m]
    (merge (zipmap (index->idxvec index) components)
           {:offset 0}
           (dissoc m :components))))
(defmethod api->client-req* :index-pull [_ m] m)
...
(defmethod client-resp->api* [:q :qseq] [_ _ m] m)
...
(defmethod client-resp->api* :index-pull [_ _ m] (or (:data m) {}))
